...
// NewDispatcher builds a B3 dispatcher pre-wired with all language detectors.
// Each detector is internally extension-gated, so the dispatcher just calls
// each in turn; non-matching detectors return nil cheaply.
...
func (d *Dispatcher) Name() string { return "b3" }
