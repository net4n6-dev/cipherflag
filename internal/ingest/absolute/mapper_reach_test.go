...
func TestMapReachOutput_ParsesNDJSONIntoDiscoveryResult(t *testing.T) {
	data, err := os.ReadFile("testdata/reach_results_sample.ndjson")
	if err != nil {
		t.Fatalf("read fixture: %v", err)
	}
	result, parseErrs, err := MapReachOutput(bytes.NewReader(data), "absolute", "host-1")
	if err != nil {
		t.Fatalf("MapReachOutput: %v", err)
	}
	if len(parseErrs) != 0 {
	...
	if result.Source != "absolute" || result.Hostname != "host-1" {
		t.Errorf("envelope = %+v", *result)
	}
}

func TestMapReachOutput_ReportsMalformedLines(t *testing.T) {
	body := strings.NewReader(`{"type":"library","name":"openssl","version":"3"}` + "\n" + `{broken json` + "\n")
	result, parseErrs, err := MapReachOutput(body, "absolute", "h")
	if err != nil {
		t.Fatalf("MapReachOutput: %v", err)
	}
